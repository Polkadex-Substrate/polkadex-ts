...
export class Bifrost implements BaseChainAdapter {
  ...
  async getBalances(address: string, assets: Asset[]): Promise<AssetAmount[]> {
    ...
    const balances = assets.map(async (a): Promise<AssetAmount> => {
      ...
      // Native asset
      if (!a.id || Object.keys(a.id).includes("Native")) {
        ...
        // Non-native asset
        const nonNative = await api.query.tokens.accounts(address, a.id);
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        // @ts-ignore
        const raw = BigInt(nonNative.toJSON()?.free || "0");
        amount = Number(Utils.formatUnits(raw, a.decimal));
      }
